...
function init() {
    ...
    // Adicionar HTML dinamicamente (Checkbox e botão de Preview)
    ...
    if (spriteSheetPanelDiv && !document.getElementById('captureIsometricToggle')) {
        ...
        spriteSheetPanelDiv.insertBefore(recordBtn, spriteSheetPanelDiv.lastChild.nextSibling); // Insere antes do último filho
        spriteSheetPanelDiv.insertBefore(isoDiv, recordBtn);
        ...
        // Insere o botão de preview antes do botão de gravar
        spriteSheetPanelDiv.insertBefore(previewButton, recordBtn);
    }
    ...
    recordBtn.addEventListener('click', startSpriteSheetRecording); // <- MUDANÇA FUTURA AQUI
...
function updateOrthoCameraView() {
    ...
    const aspect = renderCanvas.clientWidth / renderCanvas.clientHeight || 1;
    orthoCamera.left = -orthoZoomFactor * aspect;
    orthoCamera.right = orthoZoomFactor * aspect;
...
function createCameraControlsUI() {
    ...
    if (panel) panel.remove(); // Remove painel existente para evitar duplicatas
...
// --- Funções de Captura de Sprite Sheet ---
...
function captureAndGenerateSheetForAngle(filenameForSheet, totalAnglesParam, currentIndexParam) {
    return new Promise((resolve, reject) => {
        ...
        const localAnimationDuration = currentAction.getClip().duration+1; 
...
function generateSpriteSheetImage(filename, framesArrayToUse) {
    console.log(`[SPRITE GEN] Iniciando para: ${filename} | Frames: ${framesArrayToUse.length}`);
    const numFrames = framesArrayToUse.length;
    if (numFrames === 0) {
        showStatus(`Nenhum frame para gerar a sprite sheet: ${filename}.`, "error"); return;
    }
    ...
    for (let i = 0; i < numFrames; i++) {
        ...
        if (!frameImg || !(frameImg instanceof HTMLImageElement) || !frameImg.complete || frameImg.naturalWidth === 0) {
            console.error(`[SPRITE GEN] Frame inválido no índice ${i} para ${filename}. Pulando.`); continue; 
        }
        ...
        if (optimize) { 
            ...
            if (!bounds || typeof bounds.width !== 'number') {
                console.error(`[SPRITE GEN] Bounds inválidos para frame ${i}. Desenhando frame inteiro.`);
    ...
    if (!dataURL || dataURL.length < 100) {
        console.error(`[SPRITE GEN] DataURL gerado para ${filename} é inválido.`); return;
    }
    console.log(`[SPRITE GEN] DataURL da sprite sheet final gerado para ${filename}. Preparando download.`);
    ...
    const metadata = {
        ...
        angle: filename.includes("_angle") ? filename.substring(filename.lastIndexOf("_angle") + "_angle".length) : (filename.includes("_CurrentView") ? "CurrentView" : "Default"),
...
function analyzeFrameContent(frameImage) { 
    if (!frameImage || typeof frameImage.naturalWidth === 'undefined' || frameImage.naturalWidth === 0) {
        const cellW = parseInt(cellWidthInput.value) || 256;
        const cellH = parseInt(cellHeightInput.value) || 256;
        return { left: 0, top: 0, width: cellW, height: cellH };
    }
    const tempCanvas = document.createElement('canvas');
    const tempCtx = tempCanvas.getContext('2d');
    tempCanvas.width = frameImage.naturalWidth;
    tempCanvas.height = frameImage.naturalHeight;
    try { tempCtx.drawImage(frameImage, 0, 0); }
    catch (e) { return { left: 0, top: 0, width: tempCanvas.width, height: tempCanvas.height }; }
    let pixelData;
    try { pixelData = tempCtx.getImageData(0, 0, tempCanvas.width, tempCanvas.height).data; }
    catch (e) { return { left: 0, top: 0, width: tempCanvas.width, height: tempCanvas.height }; }
    ...
    if (!foundContent) return { left: 0, top: 0, width: frameImage.naturalWidth, height: frameImage.naturalHeight };
    ...
    return { left: minX, top: minY, width: Math.max(1, (maxX - minX + 1)), height: Math.max(1, (maxY - minY + 1)) };
}
// --- Initialize ---
init();